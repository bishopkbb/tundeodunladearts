...
export async function POST(request: NextRequest) {
  try {
    ...
    const orderData = {
      ...
      payment_status: (validatedData.paymentTransactionId ? 'completed' : 'pending') as 'pending' | 'processing' | 'completed' | 'failed' | 'refunded',
      order_status: (validatedData.paymentTransactionId ? 'confirmed' : 'pending') as 'pending' | 'processing' | 'confirmed' | 'shipped' | 'delivered' | 'cancelled',
